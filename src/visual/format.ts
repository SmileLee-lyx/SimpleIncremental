...
function format_raw(sign: number, mag: number, layer: number): string {
    ...
    if (layer === 0) {
        if (mag >= 0.01 && mag < 1e4) return mag.toPrecision(4);
        else return format_pow10(Math.log10(mag), 4);
    }
    ...
    if (layer === 1) {
        return format_pow10(mag * mag_sign, 4);
    }
    if (layer <= 5) {
        let result = "e";
...
declare global {
    interface Window {
        format: (num: DecimalSource) => string;
    }
}
